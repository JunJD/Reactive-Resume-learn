  ...
  return (
    <section id="hero" className="relative">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:flex lg:h-screen lg:items-center lg:px-12">
        ...
        >
          <div
            className="hidden cursor-pointer items-center gap-x-4 sm:flex "
            onClick={handleToVipPage}
          >
            <Badge className="border-amber-800 bg-amber-800 text-amber-400">{t`Become a premium member`}</Badge>

            <span
              className={cn(
                buttonVariants({ variant: "link" }),
                "space-x-2 text-left text-amber-800",
              )}
            >
              <p>{t`Learn about exclusive privileges of Premium members`}</p>
              <ArrowRight />
            </span>
          </div>
          ...
          <div className="mt-10 space-y-2">
            ...
            <h1 className="text-4xl font-bold tracking-tight sm:text-6xl">
              {t`Resume builder for new professionals`}
            </h1>
          </div>

          <p className="prose prose-base prose-zinc mt-6 text-lg leading-8 dark:prose-invert">
            {t`A resume generator that supports ai agents, a large number of templates to choose from.`}
          </p>
