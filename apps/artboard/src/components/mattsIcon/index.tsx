            ...
            <svg width={colNum * 2} height={rowNum * 2} viewBox={`0 0 ${colNum * 2} ${rowNum * 2}`} className="absolute left-0 top-0">

                {
                    Array.from({ length: rowNum - 1 }).fill(0).map((_, i) => {
                        return (
                            <line x1="0" y1={2 * (i + 1)} x2={colNum * 2} y2={2 * (i + 1)} stroke={color} stroke-width="0.3" />
                        )
                    })
                }

                {
                    Array.from({ length: colNum - 1 }).fill(0).map((_, i) => {
                        return (
                            <line x1={2 * (i + 1)} y1="0" x2={2 * (i + 1)} y2={rowNum * 2} stroke={color} stroke-width="0.3" />
                        )
                    })
                }

                <line x1="0" y1="0" x2={colNum * 2} y2="0" stroke={color} stroke-width="0.3" />
                <line x1="0" y1="0" x2="0" y2={rowNum * 2} stroke={color} stroke-width="0.3" />
            </svg>
